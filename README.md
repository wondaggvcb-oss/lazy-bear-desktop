...
| 文件 | 平台 | 怎么用 |
|------|------|--------|
| `LazyBear-Android.apk` | Android 手机 | 直接安装（[安装教程](android_bear/README.md)） |
...
- Android 版会保存到 SharedPreferences。
...
已经支持，和 macOS/Windows 一样用 DeepSeek API。首次点聊天气泡时输入 API Key，之后保存在本地，不用重复输入。
